...
int main() {
    ...
    serverAddr.sin_port = htons(PORT);    // Asignación de puerto
    ...
    listen(serverSocket, SOMAXCONN);      // Habilita el socket para recibir peticiones de conexión (socket servidor)

    std::cout << "Server listening in port " << PORT << "\n";

    std::vector<std::thread> threads;       // Vector de hilos
    while (true) {
        sockaddr_in clientAddr;     // Estructura con IP+puerto del cliente
